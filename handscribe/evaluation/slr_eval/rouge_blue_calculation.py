...
def compute_rouge_bleu_batch(references, candidates, bleu_n=(1, 2, 3, 4)):
    results = []

    for ref, hyp in zip(references, candidates):
        instance_scores = {}
        for n in bleu_n:
            scores = compute_rouge_bleu(ref, hyp, bleu_n=n, log_each_instance=False)
            instance_scores.update(scores)  # Merge scores for different n values
        results.append(instance_scores)

    return results


def compute_rouge_bleu(reference, candidate, bleu_n, log_each_instance=True):

    # BLEU-n
    # These determine how much each n-gram contributes to the final score
    weights = tuple([1.0 / bleu_n] * bleu_n)
    bleu_score = sentence_bleu(
        [reference.split()], candidate.split(),
        weights=weights, smoothing_function=SmoothingFunction().method1
    )
    ...
    if log_each_instance:
        print(f"BLEU-{bleu_n}: {bleu_score:.4f} || ROUGE-L: {rouge_l:.4f}")

    return {
        f'BLEU-{bleu_n}': bleu_score,
        'ROUGE-L': rouge_l
    }
